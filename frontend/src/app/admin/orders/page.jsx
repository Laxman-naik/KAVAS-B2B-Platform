    ...
    <div className="p-4 md:p-8 bg-[#0b1220] min-h-screen text-white">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        ...
        <div className="flex gap-3">
          ...
          />
          <button className="px-4 py-2 rounded-lg bg-gray-700 hover:bg-gray-600 transition cursor-pointer duration-200">Export CSV</button>
        </div>
      </div>
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-[#111827] text-gray-400">
            <tr>
              ...
              <th className="px-8 py-3 text-left">Buyers</th>
              <th className="px-5 py-3 text-left">Values</th>
            ...
            {filtered.length > 0 ? (
              paginatedOrders.map((o) => (
                <tr key={o.id} className="border-t border-gray-800">
                  ...
                  <td className="px-6 py-4 text-orange-400">₹{Number(o.total_amount || 0).toLocaleString("en-IN")}</td>
                  <td className="px-6 py-4">{new Date(o.created_at).toLocaleDateString()}</td>
                  <td className="px-6 py-4"><span className="px-3 py-1 rounded bg-green-500/20 text-green-400">{o.status}</span></td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan="6" className="text-center py-6 text-gray-400">{loading ? "Fetching..." : "No orders found"}</td>
              </tr>
            )}
          </tbody>
        </table>
