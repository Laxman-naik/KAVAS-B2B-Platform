...
import { CircleCheck, ChevronRight, HandCoins, RefreshCcw, Truck } from "lucide-react";
    ...
    <div className="flex items-center justify-between gap-4 py-1">
      <div className="flex items-center gap-4">
        <div className="flex h-10 w-10 items-center justify-center">
          <Icon className="h-8 w-8" style={{ color: COLORS.text, opacity: 0.7 }} />
        </div>
...
export default function ShippingDeliverySection() {
  ...
  return (
    <>
      <section className="rounded-sm border" style={{ background: COLORS.white, borderColor: COLORS.border }}>
        <div className="p-6">
      ...
      <div
        className={`fixed inset-0 z-50 ${openMoreInfo ? "pointer-events-auto" : "pointer-events-none"}`}
        aria-hidden={!openMoreInfo}
      >
        <div
          className={`absolute inset-0 bg-black/20 transition-opacity ${openMoreInfo ? "opacity-100" : "opacity-0"}`}
          onClick={() => setOpenMoreInfo(false)}
        />

        <div
          className={`absolute right-0 top-0 h-full w-[92%] max-w-md bg-white shadow-xl transition-transform duration-200 ${openMoreInfo ? "translate-x-0" : "translate-x-full"}`}
